            ...
            <ul class="sidebar-menu">
              ...
              <li class="nav-item dropdown active">
                ...
                <ul class="dropdown-menu">
                  ...
                  <li class="active"><a class="nav-link" href="{{route('admin.channel')}}">Channel Dashboard</a></li>
                </ul>
              </li>
            </ul>
  ...
  <script>
    ...
    $(document).ready(function() {
      ...
      $('#btn_tambah_data_pesantren').on('click', function(event) {
        ...
        $.ajax({
          ...
          success: function(data) {
            var jumlah_data = data.pesantren.jumlah_data + 1
            $('#modalTambahData').modal('hide')
              var html = "<tr><td>"+ jumlah_data +"</td><td class='font-weight-600'>"+data.pesantren.nama_pesantren+"</td><td><div class='badge badge-warning'>"+data.pesantren.alamat_pesantren+"</div></td><td><a href='#' class='btn btn-warning' onclick='return confirm('Yakin Hapus "+data.pesantren.nama_pesantren+"')'><i class='fa fa-trash'></i></a><a href='#' class='btn btn-primary'><i class='fa fa-edit'></i></a></td></tr>"
              $('#tampil_table').append(html);
          }
        })
      });

      //ketika tombol edit pesantren di table di tekan
      // function modalEdit(id) {
      //   event.preventDefault()
      //   $.ajax({
      //     url: "{{url('admin/ambilData')}}",
      ...
      $('#btn_edit_data_pesantren').on('click', function(event) {
        ...
        // console.log(nama_pesantren)
        $.ajax({
          url: "{{route('admin.update')}}",
          data: {id_pesantren: id_pesantren, nama_pesantren: nama_pesantren, alamat_pesantren: alamat_pesantren},
          type: 'patch',
          dataType: "JSON",
          success: function(data) {
            $('#modalEditData').modal('hide')
            $('$table_pesantren').load();
          }
        })
      });
      ...
      $('#btn_edit_channel_pesantren').on('click', function(event) {
        ...
        var id_pesantren = $("input[name=edit_id_pesantren_channel]").val()
        $.ajax({
          url: "{{route('admin.update')}}",
          data: {id_pesantren: id_pesantren, channel_id: channel_pesantren},
          type: 'patch',
          dataType: "JSON",
          success: function(data) {
            // alert(data)
      ...
      $('#btn_tambah_channel_pesantren').on('click', function(event) {
        ...
        $.ajax({
          url: "{{ route('admin.tambah.channel') }}",
          data: tambah_channel_pesantren,
          type: 'POST',
          dataType: "JSON",
          success: function(data) {
            // alert(data.nama_pesantren.nama_pesantren)
            var nama_pesantren = data.nama_pesantren.nama_pesantren
            // console.log(data)
            var jumlah_channel = data.channel.jumlah_channel + 1
            $('#modalTambahChannel').modal('hide')
              var html = "<tr><td>"+ jumlah_channel +"</td><td class='font-weight-600'>"+nama_pesantren+"</td><td>"+data.channel.channel_id+"</td><td><a href='#' class='btn btn-warning' onclick='return confirm('Yakin Hapus "+data.channel.channel_id+"')'><i class='fa fa-trash'></i></a><a href='#' class='btn btn-primary'><i class='fa fa-edit'></i></a></td></tr>"
      ...
      function modalEditChannel(id) {
        ...
        // alert(id)
        $.ajax({
          url: "{{ route('admin.ambil.data.channel') }}",
          method: "GET",
          data: {id_pesantren: id},
          dataType: "JSON",
          success: function(data) {
            console.log(data.nama_pesantren[0].nama_pesantren)
            var nama_pesantren = data.nama_pesantren[0].nama_pesantren
